...
public class Controller {
    ...
    public Controller(Model model, View view){
        ...
        view.addExitListener(new ExitSoftwareListener());
        view.addHelpListener(new ShowHelpDialouge());
    }
    ...
    class SortGroupListener implements ActionListener{
        public void actionPerformed(ActionEvent e) {
            String input = JOptionPane.showInputDialog(main, "Please enter number of groups (between 2 - number of students):", "");
            if(Util.isNumber(input)){
                int numOfGroups = Integer.valueOf(input);
                ...
                if(numOfGroups > 1 && numOfGroups <= studentList.size()){
    ...
    class ShowHelpDialouge implements ActionListener{
        public void actionPerformed(ActionEvent e) {
            JOptionPane.showMessageDialog(main, "<HTML>How to use system: <p> 1) Create a new class using 'add class' button on the right of the screen<p>2) Add students using 'add student' button on right side of the screen<p> 3) Sort class using the 'sort class' button on"
                    + " the right side of the screen<p> 4) Save class as text file using 'export class' button on right side of the screen <p><p>How sorting algorithm works: <p> 1) Sorts students based on highest number of preferences to lowest number of preferences<p> 2) "
                    + "Loops through list of groups and adds students from the list into the groups which are missing one of the studentes preferences <p> 3) if all four preferences in group have been placed, add student into the group with least number of preferences</HTML>");
        }
    }
    
    class ExitSoftwareListener implements ActionListener{
        public void actionPerformed(ActionEvent e) {
            System.exit(0);
        }
    }
